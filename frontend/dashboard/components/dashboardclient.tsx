...
function LogCallModal({ onClose, onSaved }: { onClose:()=>void; onSaved:(c:CallRow)=>void }) {
  ...
  // Duration: stored as "H:MM:SS" e.g. "1:30:00"
  const durationStr = form.duration && form.duration !== '::' && form.duration !== '0:00:00' ? form.duration : ''
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
              ...
              <div>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:'6px' }}>
                  {/* Hours 0–23 */}
                  <input type="text" inputMode="numeric" pattern="[0-9]*" placeholder="H"
                    maxLength={2}
                    ...
                    onChange={e => {
                      ...
                      upd('duration', `${h}:${parts[1]??'00'}:${parts[2]??'00'}`)
                    }}
                    style={{ ...inp, width:'56px', textAlign:'center' as const }}
                    onFocus={fo} onBlur={bl}
                  />
                  <span style={{ color:'var(--text-3)', fontWeight:700, fontSize:'16px', lineHeight:1, flexShrink:0 }}>:</span>
                  {/* Minutes 0–59 */}
                  <input type="text" inputMode="numeric" pattern="[0-9]*" placeholder="MM"
                    maxLength={2}
                    ...
                    onChange={e => {
                      ...
                      const parts = form.duration.split(':')
                      upd('duration', `${parts[0]??'0'}:${String(m).padStart(2,'0')}:${parts[2]??'00'}`)
                    }}
                    style={{ ...inp, width:'56px', textAlign:'center' as const }}
                    onFocus={fo} onBlur={bl}
                  />
                  <span style={{ color:'var(--text-3)', fontWeight:700, fontSize:'16px', lineHeight:1, flexShrink:0 }}>:</span>
                  {/* Seconds 0–59 */}
                  <input type="text" inputMode="numeric" pattern="[0-9]*" placeholder="SS"
                    maxLength={2}
                    value={form.duration.split(':')[2] ?? ''}
                    onChange={e => {
                      const raw = e.target.value.replace(/[^0-9]/g,'').slice(0,2)
                      const s = Math.min(59, parseInt(raw)||0)
                      const parts = form.duration.split(':')
                      upd('duration', `${parts[0]??'0'}:${parts[1]??'00'}:${String(s).padStart(2,'0')}`)
                    }}
                    style={{ ...inp, width:'56px', textAlign:'center' as const }}
                    onFocus={fo} onBlur={bl}
                  />
                  <span style={{ fontSize:'11px', color:'var(--text-3)', marginLeft:'4px', flexShrink:0 }}>H · MM · SS</span>
                </div>
              </div>
              ...
              <div>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'10px' }}>
                  ...
                  <input
                    ...
                    style={{ width:'58px', padding:'4px 4px', borderRadius:'7px', fontSize:'22px', fontWeight:900, letterSpacing:'-0.04em', textAlign:'center' as const, outline:'none', fontFamily:'inherit', background:'transparent', color:scoreColor(form.score), border:'none', transition:'color 0.15s' }}
                  />
                </div>
