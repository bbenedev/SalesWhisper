    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ width: '100%', maxWidth: '400px', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid rgba(139,157,181,0.12)', borderRadius: '16px', padding: '32px', boxShadow: '0 8px 40px rgba(0,0,0,0.3)' }}>
          ...
          {error && <div style={{ padding: '10px 14px', borderRadius: '8px', background: 'var(--red-dim)', border: '1px solid var(--red-border)', color: 'var(--red)', fontSize: '12.5px', marginBottom: '20px' }}>{error}</div>}
          <div style={{ marginBottom: '16px' }}>
            <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text-3)', marginBottom: '7px' }}>Email</label>
            <input type="email" placeholder="you@company.com" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleLogin()}
              style={inp} onFocus={e => (e.target.style.borderColor = 'var(--accent)')} onBlur={e => (e.target.style.borderColor = 'rgba(139,157,181,0.2)')} />
          </div>
          <div style={{ marginBottom: '8px', position: 'relative' }}>
            ...
            <input type={showPw ? 'text' : 'password'} placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleLogin()}
              style={{ ...inp, paddingRight: '42px' }} onFocus={e => (e.target.style.borderColor = 'var(--accent)')} onBlur={e => (e.target.style.borderColor = 'rgba(139,157,181,0.2)')} />
            <button onClick={() => setShowPw(p => !p)} tabIndex={-1} style={{ position: 'absolute', right: '12px', bottom: '11px', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-3)', padding: 0 }}>
              {showPw
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
            <div onClick={() => setRemember(p => !p)} style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
              ...
              <span style={{ fontSize: '12px', color: 'var(--text-3)', userSelect: 'none' }}>Remember me</span>
            </div>
          ...
          <button onClick={handleLogin} disabled={loading} style={{ width: '100%', padding: '12px', borderRadius: '9px', fontSize: '13px', fontWeight: 700, background: 'linear-gradient(135deg, #8B9DB5, #7A8EA6)', color: '#0A0F1C', border: 'none', cursor: loading ? 'wait' : 'pointer', fontFamily: 'inherit', opacity: loading ? 0.7 : 1 }}>
